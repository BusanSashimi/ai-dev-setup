...
- **기본 (`/run [모듈명]`)**:
  ...
  - **pages/**: `list.vue`, `list-search.vue`, `list-table.vue` (목록 페이지)
  - **pages/**: `detail.vue` (상세 페이지, 페이지 방식일 때)
  - **modals/**: `[모듈명]-detail.modal.vue`, `[모듈명]-insert.modal.vue`, `[모듈명]-update.modal.vue` (모달 방식)

- **투뎁스 (`/run [모듈명] two`)**:

  - **pages/**: `list.vue` (좌측: 검색+목록, 우측: 상세뷰)
  - **pages/**: `list-search.vue`, `list-table.vue` (좌측 패널용)
  - **pages/**: `list-detail.vue` (우측 상세 패널, 탭 구조 포함)
  - **modals/**: `[모듈명]-insert.modal.vue`, `[모듈명]-update.modal.vue` (모달 방식)

- **선택 모달 (`/run [모듈명] select`)**:
  - **modals/**: `[모듈명]-select.modal.vue` (선택 모달 컴포넌트)
  - **pages/**: `demo.vue` (선택 모달 사용 예제 페이지)
...
- **Pages 폴더 파일**: `[모듈명]-` 접두사 + 파일명 형식 사용
  - 리스트: `[모듈명]-list.vue`, `[모듈명]-list-search.vue`, `[모듈명]-list-table.vue`
  - 상세: `[모듈명]-detail.vue`
  - 등록: `[모듈명]-insert.vue`
  - 수정: `[모듈명]-update.vue`
  - 기타: `[모듈명]-demo.vue`, `[모듈명]-list-detail.vue` 등
- **모달 컴포넌트**: `[모듈명]-` 접두사 + `.modal.vue` 접미사 사용
  - 상세 모달: `[모듈명]-detail.modal.vue`
  - 등록 모달: `[모듈명]-insert.modal.vue`
  - 수정 모달: `[모듈명]-update.modal.vue`
  - 선택 모달: `[모듈명]-select.modal.vue`
- **Validator**: `_[모듈명].validator.ts` (modals 폴더에 위치)
- **라우터**: `_[모듈명].routes.ts` (pages 폴더에 위치)
...
```
webapp/src/modules/[모듈명]/
├── pages/           # 라우팅되는 페이지 컴포넌트
├── modals/          # 모달 컴포넌트 (.modal.vue)
├── component/       # 재사용 가능한 일반 컴포넌트
├── store/           # 상태 관리
├── type/            # 타입 정의
└── test/            # 테스트 코드
```
...
- **pages/**: 실제 라우팅되는 페이지 (list.vue, detail.vue 등)
- **modals/**: 팝업/다이얼로그 (.modal.vue suffix 사용)

### 컴포넌트 구조 및 기준 코드

**기본 CRUD 패턴 (`/run [모듈명]`)**:

- **기준 폴더**: `webapp/src/modules/test-data/pages/crud/` 폴더의 구조를 **반드시** 그대로 따름
...
- **모달 컴포넌트**: 등록/수정/삭제 등은 `modals/` 폴더에 `.modal.vue` 접미사로 생성

**투뎁스 레이아웃 패턴 (`/run [모듈명] two`)**:

- **기준 폴더**: `webapp/src/modules/test-data/pages/two-depth/` 폴더의 구조를 **반드시** 그대로 따름
...
- **기준 폴더**: `webapp/src/modules/test-data/pages/select-list/` 및 `webapp/src/modules/test-data/modals/` 폴더의 구조를 **반드시** 그대로 따름
- **`[모듈명]-select.modal.vue`**: 모달 기반 선택 컴포넌트 (검색, 목록, 체크박스 선택 기능) - `modals/` 폴더에 생성
- **`demo.vue`**: 선택 모달 사용 예제 및 테스트 페이지 - `pages/` 폴더에 생성
...
- **생성 계획 (선택 모달 패턴 - `select` 옵션 사용 시):**
  - `paging()` 함수 존재 시 → 선택 모달 파일 2개 (`[모듈명]-select.modal.vue`, `demo.vue`) 생성 계획
  - 마지막으로, 생성된 컴포넌트를 위한 `[모듈명].routes.ts` 생성 계획
  - **주의:** 선택 모달 패턴에서는 `insert()`, `update()`, `softDelete()` 관련 컴포넌트를 생성하지 않음 (선택 전용)

### 2단계: UI 페이지 및 Validator 파일 생성 (조건부)

사전 분석 결과를 바탕으로, 다음 경로에 필요한 파일을 생성합니다:

- **페이지 컴포넌트**: `webapp/src/modules/[모듈명]/pages/`
- **모달 컴포넌트**: `webapp/src/modules/[모듈명]/modals/`
- **일반 컴포넌트**: `webapp/src/modules/[모듈명]/component/`

#### 기본 패턴 컴포넌트 생성

- **기준 코드:** 모든 파일은 `webapp/src/modules/test-data/pages/crud/` 폴더의 파일을 기준으로 작성합니다.

1.  **`[모듈명]-list.vue`, `[모듈명]-list-search.vue`, `[모듈명]-list-table.vue` 생성 (`paging()` 함수 존재 시)**
    ...
    - **`[모듈명]-list.vue`**:
      - 위에서 생성한 `[모듈명]-list-search.vue`와 `[모듈명]-list-table.vue`를 포함하는 껍데기(shell) 컴포넌트입니다.

2.  **`[모듈명]-detail.vue` 생성 (`detail()` 함수 존재 시)**
...
- **기준 코드:** 모든 파일은 `webapp/src/modules/test-data/pages/two-depth/` 폴더의 파일을 기준으로 작성합니다.

1.  **`[모듈명]-list.vue` 생성 (투뎁스 레이아웃)**

    - 좌측: `[모듈명]-list-search.vue`와 `[모듈명]-list-table.vue`를 포함하는 좌측 패널 (고정 최대 너비 350px)
    - 우측: `[모듈명]-list-detail.vue`를 포함하는 우측 상세 패널 (가변 너비)
    - Flexbox 레이아웃을 사용하여 두 패널을 수평 배치합니다.

2.  **`[모듈명]-list-search.vue`, `[모듈명]-list-table.vue` 생성 (좌측 패널용)**

    - **`[모듈명]-list-search.vue`**: 기본 패턴과 동일하게 검색 폼 구현
    - **`[모듈명]-list-table.vue`**: 기본 패턴과 유사하지만, 행 클릭 시 우측 상세뷰를 업데이트하는 기능 추가
      - 행 클릭 시 `router.push({ query: { ...route.query, selected: item.id } })` 방식으로 선택 상태 관리

3.  **`[모듈명]-list-detail.vue` 생성 (우측 패널용)**
...
- **기준 코드:** 모든 파일은 `webapp/src/modules/test-data/pages/select-list/` 및 `webapp/src/modules/test-data/modals/` 폴더의 파일을 기준으로 작성합니다.

1.  **`[모듈명]-select.modal.vue` 생성 (`paging()` 함수 존재 시)**

    - **생성 경로**: `webapp/src/modules/[모듈명]/modals/[모듈명]-select.modal.vue`
    - 모달 기반 선택 컴포넌트 구현
...
2.  **`[모듈명]-demo.vue` 생성**
    ...
    - **생성 경로**: `webapp/src/modules/[모듈명]/pages/[모듈명]-demo.vue`
...
1.  **`[모듈명]-insert.modal.vue` 생성 (`insert()` 함수 존재 시)**

    - **생성 경로**: `webapp/src/modules/[모듈명]/modals/[모듈명]-insert.modal.vue`
    - `[ModuleName]InsertDto` 타입을 활용하여 입력 폼을 구성합니다.
    - '저장' 버튼 클릭 시 Store의 `insert()` 함수를 호출합니다.
    - 아래에서 생성할 Validator를 적용하여 유효성 검사를 수행합니다.

2.  **`[모듈명]-update.modal.vue` 생성 (`update()` 함수 존재 시)**

    - **생성 경로**: `webapp/src/modules/[모듈명]/modals/[모듈명]-update.modal.vue`
...
3.  **`[모듈명]-detail.modal.vue` 생성 (`detail()` 함수 존재 시, 기본 패턴에서 모달 사용 시)**

    - **생성 경로**: `webapp/src/modules/[모듈명]/modals/[모듈명]-detail.modal.vue`
    - Store의 `detailData` 상태를 연동합니다.
    - `UFormField`를 사용하여 상세 데이터를 표시합니다.

4.  **`_[모듈명].validator.ts` 생성 (`insert()` 또는 `update()` 함수 존재 시)**
    - **생성 경로**: `webapp/src/modules/[모듈명]/modals/_[모듈명].validator.ts`
    - 기본 패턴: `webapp/src/modules/test-data/pages/crud/_crud.validator.ts` 파일을 참고
...
1.  **`_[모듈명].routes.ts` 파일 생성**

    - `webapp/src/modules/[모듈명]/pages/_[모듈명].routes.ts` 경로에 생성합니다.
    - **실제 생성된 페이지 컴포넌트만** 라우트 설정에 포함시킵니다.
    - ❗️`modals/` 폴더의 모달 컴포넌트들은 라우터에 등록하지 않습니다.
    - 일반적으로 `pages/` 폴더의 `[모듈명]-list.vue`, `[모듈명]-detail.vue`, `[모듈명]-demo.vue` 등만 라우트로 등록됩니다.
...
- [ ] `webapp/src/modules/[모듈명]/pages/[모듈명]-list.vue` (`paging()` 함수 존재 시)
- [ ] `webapp/src/modules/[모듈명]/pages/[모듈명]-list-search.vue` (`paging()` 함수 존재 시)
- [ ] `webapp/src/modules/[모듈명]/pages/[모듈명]-list-table.vue` (`paging()` 함수 존재 시)
- [ ] `webapp/src/modules/[모듈명]/pages/[모듈명]-detail.vue` (`detail()` 함수 존재 시, 페이지 방식으로 생성 시)

**모달 컴포넌트 (modals/):**

- [ ] `webapp/src/modules/[모듈명]/modals/[모듈명]-detail.modal.vue` (`detail()` 함수 존재 시, 모달 방식으로 생성 시)
- [ ] `webapp/src/modules/[모듈명]/modals/[모듈명]-insert.modal.vue` (`insert()` 함수 존재 시)
- [ ] `webapp/src/modules/[모듈명]/modals/[모듈명]-update.modal.vue` (`update()` 함수 존재 시)

**기타 파일:**

- [ ] `webapp/src/modules/[모듈명]/modals/_[모듈명].validator.ts` (`insert()` 또는 `update()` 함수 존재 시)
- [ ] `webapp/src/modules/[모듈명]/pages/_[모듈명].routes.ts` (생성된 페이지 컴포넌트 기준)
...
- [ ] `webapp/src/modules/[모듈명]/pages/[모듈명]-list.vue` (`paging()` 함수 존재 시 - 투뎁스 레이아웃)
- [ ] `webapp/src/modules/[모듈명]/pages/[모듈명]-list-search.vue` (`paging()` 함수 존재 시 - 좌측 패널용)
- [ ] `webapp/src/modules/[모듈명]/pages/[모듈명]-list-table.vue` (`paging()` 함수 존재 시 - 좌측 패널용)
- [ ] `webapp/src/modules/[모듈명]/pages/[모듈명]-list-detail.vue` (`paging()` 함수 존재 시 - 우측 패널용)
...
- `[모듈명]-detail.vue` 페이지는 생성되지 않음 (`[모듈명]-list-detail.vue`가 대체)
- `[모듈명]-list.vue`는 좌우 분할 레이아웃 구조
- `[모듈명]-list-detail.vue`는 탭 구조로 상세 정보 표시
...
- [ ] `webapp/src/modules/[모듈명]/pages/[모듈명]-demo.vue` (선택 모달 테스트 페이지)

**모달 컴포넌트 (modals/):**

- [ ] `webapp/src/modules/[모듈명]/modals/[모듈명]-select.modal.vue` (`paging()` 함수 존재 시)
...
- **test-data/crud 폴더 기준**: 모든 구조와 디자인은 `webapp/src/modules/test-data/pages/crud/` 기준
- **개별 페이지 구조**: 목록, 상세, 등록, 수정 페이지가 각각 독립적으로 구성
- **라우터 기반 네비게이션**: 페이지 간 이동은 라우터 기반

#### 투뎁스 패턴 (`two` 옵션)

- **test-data/two-depth 폴더 기준**: 모든 구조와 디자인은 `webapp/src/modules/test-data/pages/two-depth/` 기준
...
- **기준 폴더**: `webapp/src/modules/test-data/pages/select-list/` (demo.vue) 및 `webapp/src/modules/test-data/modals/` ([모듈명]-select.modal.vue) 기준
...
| **파일명 예시** | `[모듈명]-list.vue`, `[모듈명]-list-search.vue`, `[모듈명]-list-table.vue` | `[모듈명]-list.vue`, `[모듈명]-list-detail.vue` | `[모듈명]-demo.vue`, `[모듈명]-select.modal.vue` |
| **기준 코드**   | test/pages/crud/ + test/modals/                                            | test/pages/two-depth/ + test/modals/            | test/pages/select-list/ + test/modals/           |
