...
- 상세: `[모듈명]-detail.modal.vue` 또는 `detail.vue`
- 등록: `[모듈명]-insert.modal.vue` 또는 `insert.vue`
- 수정: `[모듈명]-update.modal.vue` 또는 `update.vue`
...
2.  **`demo.vue` 생성**
    ...
    - **생성 경로**: `webapp/src/modules/[모듈명]/pages/demo.vue`
...
4.  **`[모듈명].validator.ts` 생성 (`insert()` 또는 `update()` 함수 존재 시)**
    - **생성 경로**: `webapp/src/modules/[모듈명]/pages/[모듈명].validator.ts`
...
1.  **`[모듈명].routes.ts` 파일 생성**
    ...
    - 일반적으로 `pages/` 폴더의 `list.vue`, `detail.vue`, `demo.vue` 등만 라우트로 등록됩니다.
...
- [ ] `webapp/src/modules/[모듈명]/pages/detail.vue` (`detail()` 함수 존재 시, 페이지 방식으로 생성 시)
...
- [ ] `webapp/src/modules/[모듈명]/[모듈명].routes.ts` (생성된 페이지 컴포넌트 기준)
